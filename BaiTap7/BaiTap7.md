...
## 14) Bài bổ sung theo yêu cầu

### Bài 14.1 - Chặn push trực tiếp vào `main`

Mục tiêu: chỉ cho phép merge qua Merge Request, không cho developer push thẳng lên `main`.

Thao tác trên GitLab:

1. Vào **Settings > Repository > Protected branches**
2. Ở mục **Protect a branch**, chọn branch `main`
3. Cấu hình:
   - **Allowed to merge**: Maintainers (hoặc role bạn muốn)
   - **Allowed to push**: **No one**
4. Bấm **Protect**

Kết quả mong đợi:

- Developer không thể `git push origin main`
- Muốn cập nhật `main` phải tạo Merge Request và được merge đúng quyền

### Bài 14.2 - Chỉ chạy CI trong Merge Request

Mục tiêu: `build` và `test` chỉ chạy khi pipeline source là MR.

Đã cấu hình trong file `.gitlab-ci.yml`:
...
1. Tạo branch mới: `feature/demo-ci`
2. Push branch này (không mở MR) → không có job CI chạy
3. Tạo Merge Request từ `feature/demo-ci` vào `main`
4. Quan sát pipeline MR: có job `build`, `test`

### Bài 14.3 - Stage deploy chỉ chạy thủ công khi bấm nút

Mục tiêu: deploy không tự chạy, chỉ chạy khi người dùng bấm nút **Play** trong pipeline.

Đã cấu hình trong `.gitlab-ci.yml`:
...
- Job `pages` chỉ xuất hiện trên branch mặc định (`main`)
- Trạng thái job ban đầu là manual (chờ người bấm)
- Chỉ khi bấm nút **Play** thì deploy mới thực thi

Checklist nộp thêm screenshot:

- Ảnh branch `main` được protect (không cho push)
- Ảnh pipeline MR có `build` + `test`
- Ảnh job `pages` ở trạng thái manual và ảnh sau khi bấm chạy
